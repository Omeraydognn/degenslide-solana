...
// try scrolling down to see the bottom nav
await page.evaluate(() => document.querySelector('.bottom-nav')?.scrollIntoView?.());
await page.waitForTimeout(600);
// click the leaderboard icon (should be second tab)
const nav = page.locator('.bottom-nav');
const tabs = nav.locator('.nav-item');
const cnt = await tabs.count();
console.log(`Found ${cnt} nav tabs`);
if (cnt >= 2) {
  await tabs.nth(1).click();
  await page.waitForTimeout(2000);
}
await page.screenshot({ path: 'scratch/screens/leaderboard.png' });
await browser.close();
